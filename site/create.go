...
func CreateSite(input CreateSiteInput) (*CreateSiteResult, error) {
	...
	if err := createAdminUser(db, input.Hostname, input.AdminEmail, input.AdminPassword, input.AdminFullName); err != nil {
		if isOwnedDB {
			db.Close()
		}
...
func createAdminUser(db *sql.DB, site, email, password, fullName string) error {
	...
	_, err = db.Exec(
		`INSERT INTO _kora_user (name, site, email, password_hash, full_name, roles)
		 VALUES (?, ?, ?, ?, ?, ?)`,
